module.exports = {
    ...
    execute: async (interaction) => {
        ...
        if (suggestion.time < Date.now()) return interaction.reply({ content: 'This suggestion has expired!', ephemeral: true });

        suggestion.upvotes = suggestion.upvotes.filter(user => user !== interaction.user.id);
        suggestion.downvotes = suggestion.downvotes.filter(user => user !== interaction.user.id);
